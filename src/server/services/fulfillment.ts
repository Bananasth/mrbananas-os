...
/**
 * Fulfil every line via the RESOLVED deduction (app.fulfil_order_item_resolved): deducts the
 * per-line order_item_ingredient (modifier-adjusted BoM) when present, else falls back to the
 * recipe — identical to fulfilOrder for non-modifier items. Used by the POS checkout.
 */
export async function fulfilOrderResolved(
  input: FulfilOrderInput,
): Promise<Result<{ fulfilled: number }, ServiceError>> {
  const gate = await getServiceContext(FULFIL_ROLES)
  if (!gate.ok) return gate
  ...
  let fulfilled = 0
  for (const id of ids) {
    const { error: fErr } = await db.rpc('fulfil_order_item_resolved', {
      p_order_item_id: id,
      p_employee_id: parsed.value.employeeId ?? null,
    })
    if (fErr) return err(serviceError('db', `order_item ${id}: ${fErr.message}`))
