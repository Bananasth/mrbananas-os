...
export const metadata: Metadata = { title: 'Recipe versions · Setup' }

const STATUS_VARIANT: Record<RecipeVersionStatus, BadgeProps['variant']> = {
  draft: 'warning',
  active: 'success',
  retired: 'neutral',
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-xl font-bold text-navy-800">
          {recipe?.name ?? 'สูตร · Recipe'}
        </h2>
        <Link href="/admin/recipes" className="text-sm text-navy-700 hover:text-banana-600">
          ← กลับ · Back
        </Link>
      </div>
      ...
      {versionsRes.value.length === 0 ? (
        ...
        versionsRes.value.map((v, idx) => {
          ...
          const ingredients = ingRes?.ok ? ingRes.value : []
