  ...
  return (
    <form action={action} className="grid gap-3 sm:grid-cols-3">
      ...
      <Field label="เวอร์ชัน · Version no." htmlFor="versionNo">
        <Input id="versionNo" name="versionNo" type="number" min="1" required placeholder="1" />
      </Field>
      <Field label="อายุ (ชม.) · Shelf life (h)" htmlFor="shelfLifeHours">
        <Input id="shelfLifeHours" name="shelfLifeHours" type="number" min="0" placeholder="48" />
      </Field>
      <Field label="ผลผลิต · Yield qty" htmlFor="yieldQty">
        <Input id="yieldQty" name="yieldQty" type="number" step="0.01" min="0" placeholder="20" />
      </Field>
      <div className="flex items-center gap-3 sm:col-span-3">
        <SubmitButton variant="cta">สร้างเวอร์ชันร่าง · Add draft version</SubmitButton>
  ...
  return (
    <form action={action} className="grid gap-2 sm:grid-cols-4 sm:items-end">
      ...
      <Field label="วัตถุดิบ · Item" htmlFor={`item-${recipeVersionId}`}>
        <Select id={`item-${recipeVersionId}`} name="itemId" required defaultValue="">
      ...
      <Field label="หน่วย · Unit" htmlFor={`unit-${recipeVersionId}`}>
        <Input id={`unit-${recipeVersionId}`} name="unit" required placeholder="g / ml" />
      </Field>
