...
-- =============================================================================
-- AUTH-MINIMAL SEED  (Section B)
-- The ONLY data inserted beyond the required role reference rows (created by
-- 0002 above, line ~174). NO catalog, products, recipes, inventory, production,
-- purchasing, or business sample data.
-- =============================================================================

-- Minimal tenant + branch: FK targets so a real Auth user can be linked below.
-- IDs match link-test-user.sql. Idempotent (safe to re-run).
...
insert into public.branch (id, tenant_id, name, timezone) values
  ('22222222-2222-2222-2222-222222222222',
   '11111111-1111-1111-1111-111111111111', 'Downtown', 'Asia/Bangkok')
on conflict (id) do nothing;

commit;

-- =============================================================================
-- LINK TEST USER  (Section C) — PARAMETERIZED.  *** DO NOT RUN YET ***
-- -----------------------------------------------------------------------------
-- This block is COMMENTED OUT on purpose. Run it as a SECOND pass, only AFTER:
--   Authentication -> Users -> Add user (email + password) -> copy the User UID.
-- Then: uncomment the 8 lines below, replace the 3 placeholders, run just this.
--   <AUTH_USER_ID>    the UID from the dashboard
--   <AUTH_USER_EMAIL> that user's email
--   <ROLE_KEY>        one of: owner | manager | staff | baker | customer
-- =============================================================================
