-- =============================================================================
-- qr-pay-3b-settle.sql — Verified settlement + owner provider-config writer (Migration 3b). APPLIED.
-- NOTE: app.qr_settle_payment here is SUPERSEDED by qr-pay-3b-fix.sql (auth_failed/error
-- RETURN instead of RAISE). Apply this first, then qr-pay-3b-fix.sql.
-- =============================================================================
begin;

-- owner-only provider config writer (only path into the deny-all payment_provider_config)
create or replace function app.set_payment_provider_config(
  p_branch_id uuid, p_provider text,
  p_promptpay_target text default null, p_webhook_secret text default null,
  p_settlement_secret text default null, p_is_active boolean default true)
...
begin
  ...
  if not exists (select 1 from public.branch where id = p_branch_id and tenant_id = v_tenant) then
    raise exception 'branch not in your tenant';
  end if;
  insert into public.payment_provider_config
    (tenant_id, branch_id, provider, promptpay_target, webhook_secret, settlement_secret, is_active)
  values (v_tenant, p_branch_id, p_provider, p_promptpay_target, p_webhook_secret, p_settlement_secret, p_is_active)
  on conflict (branch_id, provider) do update set
    promptpay_target = excluded.promptpay_target, webhook_secret = excluded.webhook_secret,
    settlement_secret = excluded.settlement_secret, is_active = excluded.is_active, updated_at = now();
end; $$;

create or replace function public.set_payment_provider_config(
  p_branch_id uuid, p_provider text, p_promptpay_target text default null,
  p_webhook_secret text default null, p_settlement_secret text default null, p_is_active boolean default true)
returns void language sql security invoker set search_path = '' as $$
  select app.set_payment_provider_config(p_branch_id, p_provider, p_promptpay_target, p_webhook_secret, p_settlement_secret, p_is_active);
$$;
grant execute on function app.set_payment_provider_config(uuid,text,text,text,text,boolean) to authenticated;
revoke all on function public.set_payment_provider_config(uuid,text,text,text,text,boolean) from public, anon;
grant execute on function public.set_payment_provider_config(uuid,text,text,text,text,boolean) to authenticated;

-- qr_settle_payment (original; auth_failed/error RAISE — superseded by qr-pay-3b-fix.sql)
...
begin
  ...
  if v_secret is null or p_settlement_secret is null or p_settlement_secret is distinct from v_secret then
    ...
    raise exception 'settlement authorization failed';
  end if;
  ...
  if v_pay.id is null then
    ...
    raise exception 'no payment intent for order';
  end if;
...
create or replace function public.qr_settle_payment(p_provider text, p_event_id text, p_tracking_token uuid, p_amount bigint, p_settlement_secret text)
returns jsonb language sql security definer set search_path = ''
as $$ select app.qr_settle_payment(p_provider, p_event_id, p_tracking_token, p_amount, p_settlement_secret); $$;

revoke all     on function public.qr_settle_payment(text,text,uuid,bigint,text) from public;
grant  execute on function app.qr_settle_payment(text,text,uuid,bigint,text)    to anon, authenticated;
grant  execute on function public.qr_settle_payment(text,text,uuid,bigint,text) to anon, authenticated;
